...
bool LCD_esp32s3::setPanelConfig() {
...
bool IRAM_ATTR LCD_esp32s3::on_color_trans_done(esp_lcd_panel_handle_t panel, const esp_lcd_rgb_panel_event_data_t *edata, void *user_ctx) {    
    LCD_esp32s3* vga = (LCD_esp32s3*)user_ctx;

    return false; 
}

bool IRAM_ATTR LCD_esp32s3::on_vsync(esp_lcd_panel_handle_t panel, const esp_lcd_rgb_panel_event_data_t *edata, void *user_ctx)
{
    return false;
}

bool IRAM_ATTR LCD_esp32s3::on_bounce_frame_finish(esp_lcd_panel_handle_t panel, const esp_lcd_rgb_panel_event_data_t *edata, void *user_ctx) {    
    LCD_esp32s3* vga = (LCD_esp32s3*)user_ctx;

    return false; 
}

bool IRAM_ATTR LCD_esp32s3::on_frame_buf_complete(esp_lcd_panel_handle_t panel, const esp_lcd_rgb_panel_event_data_t *edata, void *user_ctx) {    
    LCD_esp32s3* vga = (LCD_esp32s3*)user_ctx;
...
void LCD_esp32s3::setViewport(int x1, int y1, int x2, int y2) {
...
/*
bool IRAM_ATTR LCD_esp32s3::on_vsync(esp_lcd_panel_handle_t panel,
...
void LCD_esp32s3::swap() {
    ...
    esp_lcd_panel_draw_bitmap(_panel_handle, 0, 0, 639, 479, _buf8Auto[1]);
}
...
// Создание RGB-панели
    ...
    ret = esp_lcd_panel_draw_bitmap(_panel_handle, 0, 0, _width, _height, _buf8Auto[0]);  // Полный экран для теста
    if (ret != ESP_OK) {
    ...
    esp_lcd_rgb_panel_refresh(_panel_handle);
    //******************************************************************************
    // 
    // 
    // 
    // bool IRAM_ATTR LCD_esp32s3::on_bounce_empty(esp_lcd_panel_handle_t panel,
                                            void *bounce_buf,
                                            int pos_px,
                                            int len_bytes,
                                            void *user_ctx)
